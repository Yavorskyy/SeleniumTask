...
class TestPage(unittest.TestCase):
    ...
    @data(1,2,3,4,5)
    def test_word_is_within_page(self, value):
        home = HomePage(self.driver, self.wait)
        home.inputKeyword('python v.3.5')
        result = home.clickSearch()
        RowNumberLocator = result.getRowNumberLocator(value)
        searchResult = result.clickGoogleSearchResult(RowNumberLocator)
        searchResultText = searchResult.getTextFromSearchResult() 
